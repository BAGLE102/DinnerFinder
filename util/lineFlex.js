...
// ✅ 改成用 key（短字串）避免超過 300 字元限制
export function moreQuickItem(key, lineUserId) {
  return {
    type: 'action',
    action: {
      type: 'postback',
      label: '再 10 間',
      data: `action=explore_more&key=${encodeURIComponent(key)}&user=${encodeURIComponent(lineUserId)}`,
      displayText: '再 10 間'
    }
  };
}
